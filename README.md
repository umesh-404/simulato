# Simulato AI Exam Platform (v1.4.1)
...
## Recent progress (v1.4.1)

- **Perspective-aware `grid_map.json` transform:** Runtime option clicks map normalized capture coordinates through `GridMap.capture_to_screen_pixel()` using `scale_*` and `offset_*`. This addresses systematic vertical mis-clicks (e.g. always one option below the intended row) when the capture camera is not orthogonal to the exam screen.
- **Calibration reuse:** `calibrate_from_screenshot` preserves an existing non-naive transform when re-calibrating so grid positions and `pixel_positions` refresh without dropping a verified mapping.
- **Documentation:** Architecture and setup guides describe the transform; workflow docs clarify that option clicks do not fall back to stale static calibration when live detection fails.
